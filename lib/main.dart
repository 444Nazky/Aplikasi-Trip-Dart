  ...
  Widget build(BuildContext context) {
    return MultiProvider(
      providers: [
        ...
        // Providers - using ChangeNotifierProvider for ChangeNotifier classes
        ChangeNotifierProxyProvider<AuthRepository, AuthProvider>(
          create: (_) => AuthProvider(
            AuthRepository(storageService),
            AuthService(storageService),
          ),
          update: (_, authRepo, previous) => previous ?? AuthProvider(
            authRepo,
            AuthService(storageService),
          ),
        ),
        ChangeNotifierProxyProvider2<TripRepository, TariffRepository, TripProvider>(
          create: (_) => TripProvider(
            TripRepository(storageService, ConnectivityService()),
            TariffRepository(storageService),
            LocationService(),
          ),
          update: (_, tripRepo, tariffRepo, previous) => previous ?? TripProvider(
            tripRepo,
            tariffRepo,
            LocationService(),
          ),
